...
const SFX_FILES = { 
    ...
    dodge: STATIC_URL + "sounds/dodge.mp3", 
    block: STATIC_URL + "sounds/block.mp3", 
    skill: STATIC_URL + "sounds/skill.mp3", 
    coins: STATIC_URL + "sounds/coins.mp3", 
    forge: STATIC_URL + "sounds/forge.mp3", 
    win: STATIC_URL + "sounds/win.mp3", 
...
function playLottieEffect(targetId, animationUrl, extraClass = "") { 
    ...
    let anim = lottie.loadAnimation({ container: fxContainer, renderer: 'svg', loop: false, autoplay: true, path: animationUrl }); 
...
const CLASSES = {
    knight: { 
        id: "knight", name: "Рыцарь", icon: "🛡️", color: "#fbbf24", lore: "Танк-Рефлект. Выносливость повышает мощь Шипов.", 
        ...
        bars: { dmg: 40, def: 100, diff: 20 }, armorMult: 1.25, critDmgMult: 1.4, dodgeMult: 0.9 
    },
    berserk: { 
        id: "berserk", name: "Берсерк", icon: "🪓", color: "#ef4444", lore: "Стеклянная пушка. Сила скрыто дает Пробитие.", 
        ...
        bars: { dmg: 100, def: 30, diff: 50 }, armorMult: 0.85, critDmgMult: 1.9, dodgeMult: 1.0 
    },
    shadow: { 
        id: "shadow", name: "Тень", icon: "🗡️", color: "#a855f7", lore: "Мастер Контратаки. Ловкость дает силу ответного удара.", 
        ...
        bars: { dmg: 85, def: 20, diff: 90 }, armorMult: 1.0, critDmgMult: 2.2, dodgeMult: 1.3 
    },
    ranger: { 
        id: "ranger", name: "Следопыт", icon: "🏹", color: "#10b981", lore: "Вампир-Охотник. Удача дает Крит и огромный дроп.", 
        ...
        bars: { dmg: 70, def: 40, diff: 60 }, armorMult: 1.0, critDmgMult: 1.8, dodgeMult: 1.15 
    }
};
...
const ITEMS_DB = {
    "pot_heal_1": { id: "pot_heal_1", name: "Малое Зелье Здоровья", type: "consumable", subtype: "heal", power: 100, icon: "🧪", imageId: "39595", rarity: "rare", lvl: 1, price: 80, inShop: true, desc: "Восстанавливает 100 HP.", stats: {} },
    "pot_heal_2": { id: "pot_heal_2", name: "Великое Зелье", type: "consumable", subtype: "heal", power: 250, icon: "🏺", imageId: "44778", rarity: "epic", lvl: 5, price: 250, inShop: true, desc: "Восстанавливает 250 HP.", stats: {} },
    "scroll_fire": { id: "scroll_fire", name: "Свиток Метеорита", type: "consumable", subtype: "dmg_fire", power: 150, icon: "📜", imageId: "55316", rarity: "epic", lvl: 1, price: 150, inShop: true, desc: "Наносит 150 🔥 урона.", stats: {} },
    "scroll_ice": { id: "scroll_ice", name: "Свиток Бурана", type: "consumable", subtype: "dmg_ice", power: 150, icon: "❄️", imageId: "62365", rarity: "epic", lvl: 1, price: 150, inShop: true, desc: "Наносит 150 ❄️ урона.", stats: {} },
...
let hero = { 
    name: "Гладиатор", rating: 1000, level: 1, floor: 1, maxFloor: 1, exp: 0, expNext: 100, gold: 5000, unspentPoints: 0, gems: 0, tickets: 3, maxTickets: 3, nextTicketTime: 0, baseClass: "knight", hp: 100, maxHp: 100, 
    ...
    inventory: ["pot_heal_1", "pot_heal_1"], 
    talents: [], finalStats: {}, combatStats: {}, deathDebuffEnd: 0, setCounts: {}, flags: {}, questDate: "", quests: {}, activeAltar: null, altarOffers: {} 
};
...
const SECONDARY_STATS = [
    ...
    'lifesteal', 'counter', 'thorns',
    'dmg_fire', 'res_fire', 'dmg_ice', 'res_ice', 
    'dmg_dark', 'res_dark', 'dmg_holy', 'res_holy'
];

function createDynamicItem(baseTemplateId, targetLevel, rarity, isBoss = false, isRaid = false) {
    let baseItem = ITEMS_DB[baseTemplateId]; 
    ...
    for(let i=0; i<extraStatsCount; i++) {
        ...
        let statName = possibleStats[idx]; 
        possibleStats.splice(idx, 1);
        
        let sVal = 0;
        if (statName.startsWith('dmg_')) sVal = Math.floor(10 * finalMult);
        else if (statName.startsWith('res_')) sVal = Math.min(75, Math.floor(15 * finalMult));
        else if (['critChance', 'dodgeChance', 'lifesteal', 'counter', 'thorns'].includes(statName)) sVal = Math.floor(5 * finalMult);
        else if (statName === 'critDmg') sVal = Math.floor(15 * finalMult);
        else sVal = Math.floor(3 * finalMult);
...
function saveGame() {
    try {
        ...
        for (let key in hero.equipment) { 
            if (hero.equipment[key] && hero.equipment[key].id !== "blocked") activeItemIds.push(hero.equipment[key].id); 
        }
        
        let customItems = {}; 
        for(let key in ITEMS_DB) { 
            if((ITEMS_DB[key].dropOnly || ITEMS_DB[key].id.includes('_upg_')) && activeItemIds.includes(key)) customItems[key] = ITEMS_DB[key]; 
        }
...
async function loadGame() {
        ...
        tg.CloudStorage.getItem('tg_rpg_hero', function(err, cloudHero) { 
            if (!err && cloudHero) { 
                tg.CloudStorage.getItem('tg_rpg_custom_items', function(err2, cloudItems) { 
                    if (!err2) applyLoadedSave(cloudHero, cloudItems); 
                }); 
            } 
        }); 
    }
    ...
    try { 
        ...
        if (res.ok) { 
            ...
            if (data.status === "ok" && data.hero_data) {
                applyLoadedSave(data.hero_data, localStorage.getItem('tg_rpg_custom_items')); 
            }
        } 
    } catch(e) {}
}
...
setInterval(() => {
    ...
    if (hero.deathDebuffEnd > Date.now()) { 
        let left = Math.ceil((hero.deathDebuffEnd - Date.now())/1000); 
        let m = Math.floor(left/60); 
        let s = left%60; 
        let el = document.getElementById('ui-debuff-timer'); 
        if(el) { 
            el.innerText = `ШТРАФ СМЕРТИ: ${m}:${s<10?'0':''}${s}`; 
    ...
    if (tTimer) { 
        if (hero.tickets >= hero.maxTickets) tTimer.innerText = "Максимум билетов"; 
            ...
            let left = Math.ceil((hero.nextTicketTime - Date.now())/1000); 
            let m = Math.floor(left/60); 
            let s = left%60; 
...
function generateEnemy(floorLevel) {
    let isMegaBoss = floorLevel % 10 === 0; 
    let isMiniBoss = floorLevel % 5 === 0 && !isMegaBoss;
    let mobData; 
    if (isMegaBoss) { 
        let bossKey = floorLevel > 100 ? 100 : floorLevel; 
        mobData = BOSSES[bossKey] || BOSSES[10]; 
    } else { 
        let normalFloorCount = floorLevel - Math.floor(floorLevel / 10); 
        let mobIndex = (normalFloorCount - 1) % NORMAL_MOBS.length; 
        mobData = NORMAL_MOBS[mobIndex]; 
    }
    
    let name = mobData.name; 
    let mobImgUrl = `${STATIC_URL}mobs/B_${mobData.id}_high_resolution.png`; 
    let bgImg = isMegaBoss ? 'throne.png' : 'grave.png'; 
    let bgUrl = `${STATIC_URL}begraund/${bgImg}`;
    
    if (isMegaBoss) name = "👑 " + name; 
    else if (isMiniBoss) name = "☠️ " + name + " (Элита)";
    
    let statMult = 1 + (floorLevel * 0.04); 
    let hp = Math.floor((30 + floorLevel * 12) * statMult); 
    let atk = Math.floor((6 + floorLevel * 3) * statMult); 
    let armor = Math.floor(floorLevel * 1.2);
    ...
    let baseEnemy = { 
        name: name, floor: floorLevel, imgUrl: mobImgUrl, bgUrl: bgUrl, 
        isBoss: isMegaBoss, isMiniBoss: isMiniBoss, isRaid: false, 
        hp: hp, maxHp: hp, nextAtkZone: ["head", "chest", "legs"][Math.floor(Math.random()*3)], 
        turnCounter: 0, 
        stats: { atk: atk, armor: armor, critChance: 5, dodge: 4, armorPen: Math.floor(floorLevel / 2) } 
    };
    
    if (isMegaBoss || isMiniBoss) { 
        let elems = ['fire', 'ice', 'dark', 'holy']; 
        let randElem = elems[Math.floor(Math.random() * elems.length)]; 
        baseEnemy.stats[`dmg_${randElem}`] = Math.floor(floorLevel * 1.5); 
        baseEnemy.stats[`res_${randElem}`] = 50; 
        let weakMap = { fire:'ice', ice:'fire', dark:'holy', holy:'dark' }; 
        baseEnemy.stats[`res_${weakMap[randElem]}`] = -30; 
    }
    return baseEnemy;
}
...
function changeFloor(dir) { 
    hero.floor += dir; 
    if (hero.floor < 1) hero.floor = 1; 
    if (hero.floor > hero.maxFloor) hero.floor = hero.maxFloor; 
    playSFX('click'); 
    saveGame(); 
    initCombat(); 
}
...
window.acceptBossEvent = function(eventId) { 
    let el = document.getElementById("boss-event-overlay"); 
    if(el) el.remove(); 
...
function startCombatProper() { 
    ...
    if (title) { 
        if (enemy.isBoss) { 
            title.innerText = `МЕГА-БОСС`; 
            title.className = "combat-header boss"; 
            document.getElementById("enemy-rage-bg").style.display = "block";
        } else if (enemy.isMiniBoss) { 
            title.innerText = `ЭЛИТНЫЙ ВРАГ`; 
            title.className = "combat-header boss";
        } else { 
            title.innerText = `ОБЫЧНЫЙ ВРАГ`; 
            title.className = "combat-header";
        } 
    } 
    
    let log = document.getElementById("combat-log"); 
    if (log) log.innerHTML = `<div class="log-entry log-sys">Сражение начинается!</div>`; 
    planEnemyTurn(); 
    updateUI(); 
}
...
function startRaid(bossId) {
    ...
    if (combatMode === 'pve') { 
        savedPveEnemy = JSON.parse(JSON.stringify(enemy)); 
        savedPveState = JSON.parse(JSON.stringify(combatState)); 
    }
    ...
    enemy = { 
        name: "Рейд: " + bData.name, floor: hero.level, 
    ...
    ['fire', 'ice', 'dark', 'holy'].forEach(el => { 
        if (bData[`dmg_${el}`]) enemy.stats[`dmg_${el}`] = Math.floor(bData[`dmg_${el}`] * statMult); 
        if (bData[`res_${el}`]) enemy.stats[`res_${el}`] = bData[`res_${el}`]; 
    });
...
async function startPvP() {
    if (hero.hp <= 0 && !GOD_MODE) return alert("Герой мертв!"); 
    playSFX('click');
    ...
    try { 
        ...
        if(res.ok) { 
            let data = await res.json(); 
            if(data.status === "ok") { pvpEnemyData = data.opponent; } 
        } 
    } catch(e) {}
    ...
    if (pvpEnemyData) { 
        ...
        enemy = { name: bot.name, floor: hero.level, imgUrl: STATIC_URL + bot.img, bgUrl: STATIC_URL + "begraund/throne.png", isBoss: false, isMiniBoss: false, isRaid: false, isPlayer: true, hp: Math.floor(hero.combatStats.hp * 0.9), maxHp: Math.floor(hero.combatStats.hp * 0.9), nextAtkZone: ["head", "chest", "legs"][Math.floor(Math.random()*3)], turnCounter: 0, stats: { atk: Math.floor(hero.combatStats.damage * 0.8), armor: Math.floor(hero.combatStats.armor * 0.8), critChance: 10, dodge: 5, armorPen: Math.floor(hero.level) }, ratingReward: 25 + Math.floor(Math.random()*10) }; 
    }
...
function planEnemyTurn() { 
    ...
    if (enemy.isRaid) { 
        ...
        if (turnsLeft <= 0) enemy.nextAtkZone = 'ENRAGE'; 
        else if (enemy.turnCounter % 4 === 0) enemy.nextAtkZone = 'ULTIMATUM'; 
...
function updateIntentDisplay() { 
    ...
    if (enemy.nextAtkZone === 'ENRAGE') { 
        ...
        el.innerHTML = `🗡️ <span style="color:#a1a1aa;">Враг выжидает и готовится к удару...</span>`; 
        el.className = "intent-box"; 
    } 
}
...
function triggerClashAnim(isHero, isEnemy) { 
    if (isHero) { 
        ...
        if(hc) { hc.classList.remove("clash-hero-anim"); void hc.offsetWidth; hc.classList.add("clash-hero-anim"); } 
    } 
    if (isEnemy) { 
        let ec = document.getElementById("entity-enemy-box"); 
    ...
    if (isHero && isEnemy) { 
        ...
        if(sp) { sp.classList.remove("spark-anim"); void sp.offsetWidth; sp.classList.add("spark-anim"); } 
    } 
}

function triggerHitAnim(elementId) { 
    let el = document.getElementById(elementId); 
    if(el) { el.classList.remove("hit-anim"); void el.offsetWidth; el.classList.add("hit-anim"); } 
    let slashId = elementId === "entity-hero-box" ? "hero-slash" : "enemy-slash"; 
...
function showDmgPopup(entityBoxId, text, colorClass) { 
    let box = document.getElementById(entityBoxId); 
    if (!box) return; 
    let pop = document.createElement("div"); 
    pop.className = `dmg-popup ${colorClass}`; 
    pop.innerText = text; 
    box.appendChild(pop); 
...
function selectZone(type, zone) { 
    if(type === 'atk') combatState.atkZone = zone; 
    if(type === 'def') combatState.defZone = zone; 
    if (window.tg && tg.HapticFeedback) tg.HapticFeedback.selectionChanged(); 
    playSFX('click'); 
    updateUI(); 
}

function resetCombatZones() { 
    combatState.atkZone = null; 
    combatState.defZone = null; 
}
...
function useConsumable(itemId) {
    ...
    if (enemy.hp <= 0) { 
        ...
        setTimeout(() => { isTurnExecuting = false; handleCombatWin(); }, 400); 
    } else { 
        updateUI(); 
    }
}
...
function useClassSkill() {
    if (isTurnExecuting || combatState.skillCooldown > 0 || hero.hp <= 0) return; 
    ...
    if (hero.baseClass === 'knight') { 
        ...
        shakeScreen(); 
        let dmg = Math.floor(hero.combatStats.damage * 2.5); 
        enemy.hp -= dmg; 
        ...
        let dmg = Math.floor(hero.combatStats.damage * 1.8); 
        enemy.hp -= dmg; 
        if (enemy.isRaid) addQuestProgress('boss_dmg', dmg); 
        showDmgPopup("entity-enemy-box", `-${dmg}`, "log-crit"); 
        ...
        let dmg = Math.floor(hero.combatStats.damage * 1.5); 
        enemy.hp -= dmg; 
        if (enemy.isRaid) addQuestProgress('boss_dmg', dmg); 
        combatState.enemyStunned = true; 
        triggerHitAnim("entity-enemy-box"); 
        showDmgPopup("entity-enemy-box", `ОГЛУШЕНИЕ!`, "log-block"); 
        if (hasTalent('r5a') && enemy.isBoss) { 
            enemy.stats.armor = Math.floor(enemy.stats.armor * 0.9); 
            logCombat(`<span class="log-skill">СКИЛЛ! Броня босса снижена на 10%.</span>`); 
        } else { 
            logCombat(`<span class="log-skill">Вы применили СКИЛЛ! Враг оглушен.</span>`); 
        } 
    }
...
function calcDmg(attacker, defender, zAtk, zDef, isHeroAtk) {
    ...
    if (Math.random() * 100 < dodgeChance) return { dmg: 0, rawDmg: 0, elemLog: '', type: "dodge" };
    ...
    if (bIsShadowAtk) { 
        ...
        if (hasTalent('s5b') && defender.hp <= defender.maxHp * 0.2) return { dmg: 999999, rawDmg: 999999, elemLog: '', type: 'crit' }; 
    ...
    let elemLog = [];
    ['fire', 'ice', 'dark', 'holy'].forEach(el => { 
        let rawElemDmg = attacker[`dmg_${el}`] || 0; 
        if (rawElemDmg > 0) { 
            let res = defender[`res_${el}`] || 0; 
            let actualElemDmg = Math.floor(rawElemDmg * (1 - res/100)); 
            if (actualElemDmg > 0) { 
                elemDmgTotal += actualElemDmg; 
                let icon = el==='fire'?'🔥':el==='ice'?'❄️':el==='dark'?'☠️':'☀️'; 
                elemLog.push(`+${actualElemDmg}${icon}`); 
            } 
        } 
    });
    
    let finalDmg = physDmg + elemDmgTotal; 
    let eLogStr = elemLog.length > 0 ? ` <span style="font-size:10px;">(${elemLog.join(' ')})</span>` : '';
...
function handleCombatWin() {
    ...
    if (combatMode === 'pvp') {
        ...
        let isRaid = enemy.isRaid; 
        let logMsg = ""; 
        let goldGained = isRaid ? (hero.level * 50 + 100) : (15 + (hero.floor * 5)); 
        let expGained = isRaid ? 0 : (20 + (hero.floor * 8));
        
        if (!isRaid) {
            addQuestProgress('kill_mobs', 1); 
            if (enemy.isMiniBoss) { goldGained *= 2; expGained *= 2; } 
            if (enemy.isBoss) { goldGained *= 4; expGained *= 3; }
        ...
        if (droppedItem) { 
            if (hero.inventory.length < 15) { 
                hero.inventory.push(droppedItem.id); 
            } else { 
                hero.gold += Math.floor(droppedItem.price * 0.5); 
                logMsg += `<br>Сумка полна! ${droppedItem.name} продан за ${Math.floor(droppedItem.price * 0.5)}💰.`; 
                droppedItem = null; 
            } 
        }
        
        document.getElementById("vic-title-text").innerText = isRaid ? "РЕЙД ЗАВЕРШЕН!" : "ВРАГ ПОВЕРЖЕН!"; 
        document.getElementById("vic-rewards-text").innerHTML = `Заработано: +${goldGained}💰 ${expGained>0?`| +${expGained} EXP`:''} ${logMsg}`;
        
        let lootBox = document.getElementById("vic-loot-container"); 
        if (lootBox) { 
            if (droppedItem) { 
                lootBox.style.display = "flex"; 
                document.getElementById("vic-loot-box").className = `vic-loot-box rarity-${droppedItem.rarity}`; 
                document.getElementById("vic-loot-box").innerHTML = renderItemIcon(droppedItem); 
                document.getElementById("vic-loot-name").innerText = droppedItem.name; 
            } else { 
                lootBox.style.display = "none"; 
            } 
        }
        
        if (!isRaid && hero.floor === hero.maxFloor && hero.maxFloor < 100) { 
            hero.maxFloor++; 
            hero.floor = hero.maxFloor; 
        }
    }
    saveGame(); 
    document.getElementById("vic-modal").classList.add("show"); 
    updateUI(); 
}

function closeVictoryModal() { 
    playSFX('click'); 
    document.getElementById("vic-modal").classList.remove("show"); 
    if (combatMode === 'pvp') { 
        combatMode = 'pve'; 
        if (savedPveEnemy) { enemy = savedPveEnemy; combatState = savedPveState; savedPveEnemy = null; savedPveState = null; } 
        else { initCombat(); } 
        openScreen('arena'); 
...
function applyTurnEndEffects() {
    ...
    if (hasTalent('b3a') && hero.baseClass === 'berserk' && hero.hp > 0) { 
        hero.hp = Math.min(hero.combatStats.hp, hero.hp + 5); 
    } 
}
...
function executeTurn() {
    ...
    try {
        ...
        let eAtkZone = enemy.nextAtkZone; 
        let eDefZone = ["head", "chest", "legs"][Math.floor(Math.random()*3)];
        ...
        if (hasTalent('r1c') && hero.baseClass === 'ranger' && combatState.enemyTurns % 3 === 0) { 
            isEnemyStunned = true; 
            logCombat(`<span class="log-sys">ЛОВЧИЙ! Враг замедлен.</span>`); 
        }
        ...
        setTimeout(() => {
            try {
                ...
                if (hRes.type === "dodge") { 
                    showDmgPopup("entity-enemy-box", "УВОРОТ", "log-dodge"); 
                } else if (hRes.type === "crit") { 
                    showDmgPopup("entity-enemy-box", `КРИТ -${hRes.dmg}`, "log-crit"); 
                    if (hasTalent('b4c') && hero.baseClass === 'berserk' && combatState.zoneHealth[eDefZone] > 0) { 
                        combatState.zoneHealth[eDefZone] = Math.max(0, combatState.zoneHealth[eDefZone] - 2); 
                    } 
                    if (hero.flags.storm && heroAtkZone === 'head' && Math.random() < 0.3) { 
                        combatState.enemyStunned = true; logCombat(`<span class="log-sys">СНАЙПЕР! Враг оглушен.</span>`); 
                    } 
                ...
                if (enemy.hp <= 0) { 
                    isTurnExecuting = false; setTimeout(() => handleCombatWin(), 400); 
                } else {
                    applyTurnEndEffects(); 
                    if (enemy.hp <= 0) { isTurnExecuting = false; return; }
                    
                    if (isEnemyStunned) { 
                        logCombat(`<span class="log-sys">${enemy.name} пропускает ход.</span>`); 
                        combatState.enemyStunned = false; saveGame(); updateUI(); isTurnExecuting = false; 
                    } else {
                        setTimeout(() => {
                            try {
                                let forceDodge = hero.baseClass === 'shadow' && combatState.skillCooldown === (CLASSES.shadow.skill.cd - 1); 
                                let eRes;
                                
                                if (eAtkZone === 'ENRAGE') { 
                                    eRes = { dmg: 99999, rawDmg: 99999, elemLog: '', type: "crit" }; shakeScreen(); 
                                } else if (eAtkZone === 'ULTIMATUM') { 
                                    shakeScreen(); 
                                    let baseAtk = Math.floor((enemy.stats.atk || 5) * 2.5); 
                                    if (forceDodge) eRes = { dmg: 0, rawDmg: 0, elemLog: '', type: "dodge" }; 
                                    else { 
                                        let mitigation = Math.floor(hero.combatStats.armor * 0.2); 
                                        let finalDmg = Math.max(Math.floor(baseAtk * 0.2), baseAtk - mitigation); 
                                        eRes = { dmg: finalDmg, rawDmg: baseAtk, elemLog: '', type: "crit" }; 
                                    } 
                                } else { 
                                    eRes = calcDmg(enemy.stats, hero.combatStats, eAtkZone, heroDefZone, false); 
                                    if(forceDodge) eRes = { dmg: 0, rawDmg: 0, elemLog: '', type: "dodge" }; 
                                }
                                    ...
                                    try {
                                        ...
                                        if (eRes.type === "dodge") { 
                                            ...
                                            if (hero.baseClass === 'shadow') { 
                                                ...
                                                if (hasTalent('s4a')) counterPct += 100; 
                                                if (hasTalent('s2a')) hero.hp = Math.min(hero.combatStats.hp, hero.hp + Math.floor(hero.combatStats.hp * 0.05)); 
                                            } 
                                            if (counterPct > 0) { 
                                                let cDmg = Math.floor(hero.combatStats.damage * (counterPct / 100)); 
                                                enemy.hp -= cDmg; 
                                                if(enemy.isRaid) addQuestProgress('boss_dmg', cDmg); 
                                                showDmgPopup("entity-enemy-box", `КОНТР -${cDmg}`, "log-crit"); playSFX('crit'); 
                                                logCombat(`<span class="log-crit">Вы контратаковали на ${cDmg} урона!</span>`); 
                                            } 
                                            ...
                                            let thornsPct = hero.combatStats.thorns || 0; 
                                            if (hero.baseClass === 'knight') { 
                                                if (eRes.type === "perfect_block") thornsPct += hasTalent('k5b') ? 100 : (hasTalent('k2b') ? 50 : 20); 
                                            ...
                                            if (thornsPct > 0) { 
                                                let rDmg = Math.floor(eRes.rawDmg * (thornsPct / 100)); 
                                                enemy.hp -= rDmg; 
                                                if(enemy.isRaid) addQuestProgress('boss_dmg', rDmg); 
                                                showDmgPopup("entity-enemy-box", `ШИПЫ -${rDmg}`, "log-block"); 
                                                logCombat(`<span class="log-block">Шипы отразили ${rDmg} урона!</span>`); 
                                            } 
                                        } else if (eAtkZone === 'ENRAGE' || eAtkZone === 'ULTIMATUM') { 
                                            showDmgPopup("entity-hero-box", `УЛЬТА! -${eRes.dmg}`, "log-crit"); 
                                        } else { 
                                            showDmgPopup("entity-hero-box", `-${eRes.dmg}`, "log-dmg"); 
                                        }
                                        ...
                                        if (eRes.dmg > 0) { 
                                            ...
                                            if (eAtkZone !== 'ULTIMATUM' && eAtkZone !== 'ENRAGE' && eRes.type !== "perfect_block" && combatState.zoneHealth[eAtkZone] > 0) { 
                                                ...
                                                if (combatState.zoneHealth[eAtkZone] === 0) { 
                                                    logCombat(`<span class="log-dmg">⚠️ БРОНЯ В ЗОНЕ '${zNameRu[eAtkZone].toUpperCase()}' ПОЛНОСТЬЮ РАЗРУШЕНА!</span>`); 
                                                    showDmgPopup("entity-hero-box", "СЛОМАНО!", "log-crit"); 
                                                    calculateStats(true); 
                                                } 
                                            } 
                                        }
                                        ...
                                        if (hero.hp <= 0 && !GOD_MODE) {
                                            if (hero.flags.bloodied && !combatState.bloodiedUndying) { 
                                                hero.hp = 1; combatState.bloodiedUndying = true; combatState.bloodiedLifesteal = true; 
                                                logCombat(`<span class="log-sys">КРОВАВЫЙ ОСКАЛ! Вы выжили. След. удар лечит.</span>`); 
                                                showDmgPopup("entity-hero-box", "ЖАЖДА!", "log-sys"); planEnemyTurn(); saveGame(); updateUI(); isTurnExecuting = false; 
                                            } else if (hasTalent('b5a') && hero.baseClass === 'berserk' && !combatState.undyingUsed) { 
                                                hero.hp = 1; combatState.undyingUsed = true; 
                                                logCombat(`<span class="log-sys">БЕССМЕРТИЕ! Вы выжили с 1 HP.</span>`); 
                                                showDmgPopup("entity-hero-box", "СПАСЕН!", "log-sys"); planEnemyTurn(); saveGame(); updateUI(); isTurnExecuting = false; 
                                            } else { 
                                                hero.activeAltar = null; hero.altarOffers = {}; hero.hp = 0; 
                                                if(combatMode === 'pvp') { 
                                                    let ratingLost = 10 + Math.floor(Math.random()*10); 
                                                    hero.rating = Math.max(0, hero.rating - ratingLost); 
                                                    logCombat(`<span class="log-dmg">Вы проиграли. Рейтинг -${ratingLost} 🏆</span>`); 
                                                    playSFX('death'); updateUI(); saveGame(); isTurnExecuting = false; 
                                                    ...
                                                    logCombat(`<span class="log-dmg">💀 ВЫ ПОГИБЛИ. ТЯЖЕЛОЕ РАНЕНИЕ на 10 минут.</span>`); 
                                                    calculateStats(); playSFX('death'); updateUI(); saveGame(); isTurnExecuting = false; 
                                                    setTimeout(() => { alert("Вы отступаете в Лагерь..."); if (combatMode === 'raid') { combatMode = 'pve'; if (savedPveEnemy) { enemy = savedPveEnemy; combatState = savedPveState; savedPveEnemy = null; savedPveState = null; } else { initCombat(); } } else { enemy = null; } openScreen('hero'); }, 2000); 
                                                }
                                            }
...
function openInspectModal(invIndex) {
    ...
    if (item.setId && SETS_DB[item.setId]) { 
        let set = SETS_DB[item.setId]; 
        let count = hero.setCounts[item.setId] || 0; 
        setHtmlBlock = `<div style="margin-top:8px; border-top:1px dotted #3f3f46; padding-top:6px; font-size:10px;"><b style="color:#fbbf24;">Сет: ${set.name} (${count}/4)</b><br><span style="color:${count>=2?'#10b981':'#71717a'}">[2 шт] ${set.p2}</span><br><span style="color:${count>=4?'#10b981':'#71717a'}">[4 шт] ${set.p4}</span></div>`; 
    }
    
    document.getElementById("inspect-stats-box").innerHTML = `<div style="color: #f4f4f5; font-size: 13px; line-height: 1.6; text-shadow: 0 1px 2px rgba(0,0,0,0.8);">` + formatStats(item.stats).join(' • ') + `</div>` + setHtmlBlock + `<br><i style="color:#71717a; margin-top:4px; display:block;">${item.desc||''}</i>`;
    
    let sellPrice = Math.floor(item.price * 0.5); 
    document.getElementById("btn-inspect-sell").innerText = `ПРОДАТЬ ЗА 💰 ${sellPrice}`;
    
    let eqBtn = document.getElementById("btn-inspect-equip");
    if(item.type === 'consumable') { 
        eqBtn.innerText = "ИСПОЛЬЗОВАТЬ В БОЮ"; eqBtn.style.background = "#52525b"; eqBtn.style.boxShadow = "none"; eqBtn.onclick = null; 
    } else { 
        eqBtn.innerText = "НАДЕТЬ"; eqBtn.style.background = "#fbbf24"; eqBtn.style.boxShadow = "0 4px 10px rgba(251,191,36,0.3)"; eqBtn.onclick = function() { equipItem(inspectInvIndex); closeInspectModal(); }; 
    }
...
function closeInspectModal() { 
    playSFX('click'); 
    document.getElementById("item-inspect-modal").classList.remove("show"); 
    inspectInvIndex = null; 
}
...
function openScreen(screenName) {
    ...
    if(screenName === 'rating') { 
        ...
        if(tc) tc.innerHTML = `<div style="text-align:center; color:#71717a; padding:20px;">⏳ Обновление данных...</div>`; 
...
function changeClass(classKey) {
    ...
    for (let slot in hero.equipment) { 
        ...
        if (item && item.id !== "blocked" && item.allowedClasses && !item.allowedClasses.includes(classKey)) itemsToUnequip.push(slot); 
    }
    
    let neededSlots = (hero.inventory.length + itemsToUnequip.length) - 15; 
    if (neededSlots > 0) return alert(`Освободите ${neededSlots} мест в сумке для снятия вещей!`);
    
    if(!confirm("Внимание! При смене класса ВСЕ ВЫБРАННЫЕ ТАЛАНТЫ БУДУТ СБРОШЕНЫ. Продолжить?")) return;
    
    itemsToUnequip.forEach(slot => { 
        let item = hero.equipment[slot]; 
        if (item && item.id !== "blocked") hero.inventory.push(item.id); 
        hero.equipment[slot] = null; 
        if (item && item.type === 'two_handed') hero.equipment.weapon2 = null; 
    });
...
function equipItem(invIndex) {
    playSFX('click'); 
    if (combatMode === 'pvp' || combatMode === 'raid') return alert("Нельзя менять снаряжение на Арене или в Рейде!");
    ...
    if (item.type === 'consumable') return alert("Расходники используются в бою!");
    
    let targetSlot = item.type;
    
    if (item.type === 'two_handed') {
        let w1 = hero.equipment.weapon1; let w2 = hero.equipment.weapon2; 
        let needsExtraSlot = (w1 && w2 && w2.id !== "blocked");
        if (needsExtraSlot && hero.inventory.length >= 15) return alert("Освободите 1 место в сумке!");
        
        hero.inventory.splice(invIndex, 1); 
        if (w1) hero.inventory.push(w1.id); 
        if (w2 && w2.id !== "blocked") hero.inventory.push(w2.id);
        
        hero.equipment.weapon1 = item; 
        hero.equipment.weapon2 = { id: "blocked", icon: "🔒", name: "Занято", type: "weapon2", rarity: "common", stats: {} };
    ...
    if ((targetSlot === 'weapon1' || targetSlot === 'weapon2') && hero.equipment.weapon1 && hero.equipment.weapon1.type === 'two_handed') { 
        let twoHandedItem = hero.equipment.weapon1; 
        hero.equipment.weapon1 = null; 
        hero.equipment.weapon2 = null; 
        hero.inventory.splice(invIndex, 1); 
        hero.inventory.push(twoHandedItem.id); 
...
function unequipSlot(slotKey) {
    ...
    if (!item || item.id === "blocked") return; 
...
function addStat(statKey) { 
    if (hero.unspentPoints > 0) { 
        ...
        if (window.tg && tg.HapticFeedback) tg.HapticFeedback.selectionChanged(); 
        playSFX('click'); calculateStats(); saveGame(); updateUI(); 
    } 
}
...
function sellItem(invIndex) { 
    let item = ITEMS_DB[hero.inventory[invIndex]]; 
    if (!item) return; 
...
function healHero() {
    ...
    let cost = Math.max(10, Math.floor(missingHp * 0.5));
    if (hero.gold < cost) { 
        if (hero.gold > 0) { 
            let affordableHeal = hero.gold * 2; hero.hp += affordableHeal; hero.gold = 0; 
            alert(`Золота хватило лишь на частичное лечение (+${affordableHeal} HP).`); 
            playSFX('coins'); saveGame(); updateUI(); 
        } else { alert(`У вас нет золота!`); } 
        return; 
    }
...
function pickTalent(tierIndex, talentId) { 
    ...
    if(confirm("Вы уверены? Этот выбор навсегда определит стиль игры.")) { 
        ...
        if (window.tg && tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('success'); 
        playSFX('skill'); calculateStats(); saveGame(); updateUI(); 
    } 
}
...
function formatStats(stats) {
    ...
    if(stats.lifesteal) res.push(`<span style="color:#ef4444;">Вампиризм ${stats.lifesteal}%</span>`);
    if(stats.counter) res.push(`<span style="color:#a855f7;">Контратака ${stats.counter}%</span>`);
    if(stats.thorns) res.push(`<span style="color:#fbbf24;">Шипы ${stats.thorns}%</span>`);
    if(stats.dmg_fire) res.push(`<span class="elem-fire">Огонь ${stats.dmg_fire>0?'+':''}${stats.dmg_fire}</span>`); if(stats.res_fire) res.push(`<span class="elem-fire">Рез. Огню ${stats.res_fire>0?'+':''}${stats.res_fire}%</span>`);
    if(stats.dmg_ice) res.push(`<span class="elem-ice">Лед ${stats.dmg_ice>0?'+':''}${stats.dmg_ice}</span>`); if(stats.res_ice) res.push(`<span class="elem-ice">Рез. Льду ${stats.res_ice>0?'+':''}${stats.res_ice}%</span>`);
    if(stats.dmg_dark) res.push(`<span class="elem-dark">Тьма ${stats.dmg_dark>0?'+':''}${stats.dmg_dark}</span>`); if(stats.res_dark) res.push(`<span class="elem-dark">Рез. Тьме ${stats.res_dark>0?'+':''}${stats.res_dark}%</span>`);
    if(stats.dmg_holy) res.push(`<span class="elem-holy">Свет ${stats.dmg_holy>0?'+':''}${stats.dmg_holy}</span>`); if(stats.res_holy) res.push(`<span class="elem-holy">Рез. Свету ${stats.res_holy>0?'+':''}${stats.res_holy}%</span>`);
    return res;
}
...
function renderItemIcon(item) { 
    if (!item) return ""; 
    if (item.id === "blocked") return `<div class="item-icon">${item.icon}</div>`; 
    ...
    if (!item.imageId && item.id.includes("shields_v2_")) { let parts = item.id.split('_'); imgId = parts[0] + "_" + parts[1] + "_" + parts[2]; } 
    let folder = item.type; 
    return `<div class="item-img-wrapper"><img src="${STATIC_URL}items/${folder}/${imgId}.png" class="item-img" alt="${item.name}" onerror="this.src='${STATIC_URL}items/17271.png'"></div>`; 
}
...
function updateUI() {
    ...
    if (currentScreen === 'PVE' && enemy) {
        ...
        if (enemy.isBoss || enemy.isMiniBoss || enemy.isRaid) {
            let dropInfoHtml = enemy.isRaid ? `🏆 <b>Награда Рейда:</b> <span style="color:#c084fc">Эпик+ 100%</span> | <span style="color:#fbbf24">Шанс на Сет 25%</span>` : (enemy.isBoss ? `👑 <b>Дроп Босса (100%):</b> <span style="color:#c084fc">Эпик 50%</span> | <span style="color:#facc15">Лега 40%</span> | <span style="color:#fbbf24">Сет 5%</span>` : `☠️ <b>Дроп Элиты (30%):</b> <span style="color:#60a5fa">Редкое 60%</span> | <span style="color:#c084fc">Эпик 30%</span> | <span style="color:#facc15">Лега 10%</span>`);
            lootPreviewEl.innerHTML = `<div style="background: rgba(24, 24, 27, 0.9); border: 1px solid #3f3f46; border-radius: 8px; padding: 6px 10px; margin-bottom: 8px; font-size: 10px; text-align: center; color: #d4d4d8; box-shadow: 0 4px 10px rgba(0,0,0,0.5);">${dropInfoHtml}</div>`;
            lootPreviewEl.style.display = "block";
        } else if (lootPreviewEl) { lootPreviewEl.style.display = "none"; }
    ...
    if (currentScreen === 'hero') {
        let cls = CLASSES[hero.baseClass]; let muteBtn = `<span style="cursor:pointer; filter:grayscale(${sfxMuted?'100%':'0%'}) opacity(${sfxMuted?'0.5':'1'}); padding: 0 8px; font-size:16px;" onclick="toggleMute()">${sfxMuted?'🔇':'🔊'}</span>`;
        let cn = document.getElementById("ui-class-name"); if(cn) cn.innerHTML = `<button class="reset-btn" onclick="hardReset()">СБРОС</button> ${muteBtn} ${cls.icon} ${hero.name} <span style="font-size:11px; color:#71717a; font-weight:normal;">[${cls.name}]</span>`;
        ...
        let bc = document.getElementById("ui-bag-capacity"); if(bc) bc.innerText = `${hero.inventory.length}/15`;
        let invHtml = '';
        for (let i = 0; i < 15; i++) { 
            if (i < hero.inventory.length) { let item = ITEMS_DB[hero.inventory[i]]; if (item) { invHtml += `<div class="inv-item filled rarity-${item.rarity}" onclick="openInspectModal(${i})">${renderItemIcon(item)}</div>`; } else { invHtml += `<div class="inv-item empty"></div>`; } } 
        ...
        let elemTable = `
            <div class="stat-group-title" style="margin-top:10px;">Стихийный урон и Защита</div>
            <div class="stat-resist-grid" style="display:grid; grid-template-columns:1fr 1fr; gap:6px; background:#18181b; padding:8px; border-radius:8px; border:1px solid #27272a;">
                <div style="font-size:10px;"><span class="elem-fire">🔥 Огонь:</span> <b>+${hero.finalStats.dmg_fire || 0}</b> <br><span style="color:#71717a;">Рез: ${hero.finalStats.res_fire || 0}%</span></div>
                <div style="font-size:10px;"><span class="elem-ice">❄️ Лёд:</span> <b>+${hero.finalStats.dmg_ice || 0}</b> <br><span style="color:#71717a;">Рез: ${hero.finalStats.res_ice || 0}%</span></div>
                <div style="font-size:10px;"><span class="elem-dark">☠️ Тьма:</span> <b>+${hero.finalStats.dmg_dark || 0}</b> <br><span style="color:#71717a;">Рез: ${hero.finalStats.res_dark || 0}%</span></div>
                <div style="font-size:10px;"><span class="elem-holy">☀️ Свет:</span> <b>+${hero.finalStats.dmg_holy || 0}</b> <br><span style="color:#71717a;">Рез: ${hero.finalStats.res_holy || 0}%</span></div>
            </div>
        `;
        ...
        if(sc) sc.innerHTML = `
            ...
            <div class="stats-card">
                ...
                <div class="stat-item"><span>⚡ Крит / Урон</span> <b>${hero.finalStats.critChance}% / ${hero.finalStats.critDmg}%</b></div>
                <div style="display:flex; gap:6px; margin-top:8px; font-size:10px;">
                    <div style="flex:1; background:#27272a; padding:6px; border-radius:6px; text-align:center;"><span style="color:#ef4444">Вампиризм</span><br><b style="font-size:13px;">${hero.finalStats.lifesteal || 0}%</b></div>
                    <div style="flex:1; background:#27272a; padding:6px; border-radius:6px; text-align:center;"><span style="color:#a855f7">Контратака</span><br><b style="font-size:13px;">${hero.finalStats.counter || 0}%</b></div>
                    <div style="flex:1; background:#27272a; padding:6px; border-radius:6px; text-align:center;"><span style="color:#fbbf24">Шипы</span><br><b style="font-size:13px;">${hero.finalStats.thorns || 0}%</b></div>
                </div>
                ${elemTable}
            </div>
        `;
        ...
        RAID_BOSSES.forEach(b => {
            ...
            raidHtml += `
                <div class="raid-boss-card">
                    <div class="raid-img-box"><div class="silhouette">👾</div><img src="${imgUrl}" class="raid-img"></div>
                    <div class="raid-info">
                        <div>
                            <div class="raid-name">${b.name} <span class="entity-lvl boss" style="font-size:8px;">УР. ${hero.level}</span></div>
                            <div class="raid-desc">${b.desc} <b style="color:#fbbf24; font-size:10px;">Дроп: Сет Ур. ${tierLvl}</b> ${weakHtml}</div>
                            <div class="combat-mini-stats" style="margin-bottom: 6px; justify-content: space-between; padding: 4px 6px;"><span>❤️ ${bHp}</span> <span>⚔️ ${bAtk}</span> <span>🛡️ ${bArm}</span></div>
                        </div>
                        ${btnHtml}
                    </div>
                </div>
    ...
    if (currentScreen === 'shop') {
        ...
        if (shopMode === 'buy') {
            ...
            sortedAssortment.forEach(itemId => {
                let item = ITEMS_DB[itemId];
                let price = getShopPrice(item.price); let canAfford = hero.gold >= price; let meetLvl = hero.level >= item.lvl;
                let btnHtml = (canAfford && meetLvl) ? `<button class="shop-btn btn-buy" onclick="buyItem('${item.id}')">Купить<br>💰 ${price}</button>` : `<button class="shop-btn btn-buy" disabled>💰 ${price}</button>`;
                shopHtml += `<div class="shop-item-card"><div class="shop-item-icon rarity-${item.rarity}">${renderItemIcon(item)}</div><div class="shop-item-info"><div class="shop-item-name"><span>${item.name}</span></div><div class="shop-item-stats">${item.desc}</div></div>${btnHtml}</div>`;
            });
            ...
            if (hero.inventory.length === 0) shopHtml = `<div style="text-align:center; padding: 20px; color:#71717a;">Ваша сумка пуста. Экипировка добывается в Башне!</div>`;
            else {
                hero.inventory.forEach((itemId, index) => {
                    let item = ITEMS_DB[itemId]; if (!item) return;
        ...
        if (cd) {
            cd.innerHTML = `
                <div class="class-detail-card" style="border-color: ${selCls.color}40;">
                    <div class="class-detail-img-box"><div class="silhouette" style="position:absolute; font-size:80px; z-index:1; opacity:0.3;">👤</div><img src="${CLASS_AVATARS[previewClassId]}" alt="${selCls.name}" class="class-preview-img"><div class="class-gradient-fade"></div></div>
                    <div class="class-detail-content">
                        <div class="class-detail-header"><div class="class-detail-title" style="color:${selCls.color}; text-shadow: 0 0 15px ${selCls.color}80;">${selCls.icon} ${selCls.name}</div><div class="class-detail-growth">Авто-рост: СИЛ +${selCls.growth.str} | ЛОВ +${selCls.growth.agi} | ВЫН +${selCls.growth.end} | МСТ +${selCls.growth.mst} | УДЧ +${selCls.growth.luk}</div></div>
