...
const SFX_FILES = {
    click: STATIC_URL + "sounds/click.mp3", hit: STATIC_URL + "sounds/hit.mp3",
    crit: STATIC_URL + "sounds/crit.mp3", dodge: STATIC_URL + "sounds/dodge.mp3",
    block: STATIC_URL + "sounds/block.mp3", skill: STATIC_URL + "sounds/skill.mp3",
    coins: STATIC_URL + "sounds/coins.mp3", forge: STATIC_URL + "sounds/forge.mp3",
    win: STATIC_URL + "sounds/win.mp3", death: STATIC_URL + "sounds/death.mp3"
};
...
async function initAudio() {
    for (let key in SFX_FILES) {
        try { let res = await fetch(SFX_FILES[key]); if (!res.ok) continue; SFX_BUFFERS[key] = await audioCtx.decodeAudioData(await res.arrayBuffer()); } catch(e) {}
    }
}
initAudio();
...
function unlockAudio() {
    ...
    let buffer = audioCtx.createBuffer(1, 1, 22050); let source = audioCtx.createBufferSource();
...
function playSFX(id) {
    if (sfxMuted || !SFX_BUFFERS[id]) return; if (audioCtx.state === 'suspended') audioCtx.resume();
    try { let source = audioCtx.createBufferSource(); source.buffer = SFX_BUFFERS[id]; let gainNode = audioCtx.createGain(); gainNode.gain.value = 0.6; source.connect(gainNode); gainNode.connect(audioCtx.destination); source.start(0); } catch(e) {}
}
...
function playLottieEffect(targetId, animationUrl, extraClass = "") {
    let targetNode = document.getElementById(targetId); if (!targetNode) return;
    let fxContainer = document.createElement('div'); fxContainer.className = 'lottie-fx-layer ' + extraClass; targetNode.appendChild(fxContainer);
...
const ITEMS_DB = {
    "pot_heal_1": { id: "pot_heal_1", name: "Малое Зелье Здоровья", type: "consumable", subtype: "heal", power: 100, icon: "🧪", rarity: "rare", lvl: 1, price: 80, desc: "Восстанавливает 100 HP. В бою без траты хода.", stats: {} },
    "pot_heal_2": { id: "pot_heal_2", name: "Великое Зелье", type: "consumable", subtype: "heal", power: 250, icon: "🏺", rarity: "epic", lvl: 5, price: 250, desc: "Восстанавливает 250 HP. В бою без траты хода.", stats: {} },
    "scroll_fire": { id: "scroll_fire", name: "Свиток Метеорита", type: "consumable", subtype: "dmg_fire", power: 150, icon: "📜", rarity: "epic", lvl: 1, price: 150, desc: "Наносит 150 🔥 урона мгновенно.", stats: {} },
    "scroll_ice": { id: "scroll_ice", name: "Свиток Бурана", type: "consumable", subtype: "dmg_ice", power: 150, icon: "❄️", rarity: "epic", lvl: 1, price: 150, desc: "Наносит 150 ❄️ урона мгновенно.", stats: {} },
    ...
    "60684": { id: "60684", name: "Синий Камзол", type: "chest", icon: "🧥", rarity: "rare", lvl: 2, price: 150, stats: { armor: 8, mst: 5, luk: 2, res_ice: 10 } }, 
    ...
    "25186": { id: "25186", name: "Изумрудное Кольцо", type: "ring", icon: "💍", rarity: "rare", lvl: 3, price: 300, stats: { agi: 4, luk: 3, dodgeChance: 2, res_poison: 15 } },
    "48635": { id: "48635", name: "Стальной Стилет", type: "weapon1", icon: "🗡️", rarity: "rare", lvl: 3, price: 210, allowedClasses: ["shadow", "ranger"], dropOnly: true, stats: { atk: 18, critChance: 6, armorPen: 3, dmg_dark: 10 } }, 
    ...
    "52995": { id: "52995", name: "Пепельная Ряса", type: "chest", icon: "🥋", rarity: "epic", lvl: 4, price: 350, dropOnly: true, stats: { armor: 18, mst: 8, end: 3, res_fire: 25 } }, 
    ...
    "35056": { id: "35056", name: "Святой Клинок", type: "weapon1", icon: "⚔️", rarity: "epic", lvl: 5, price: 800, allowedClasses: ["knight"], dropOnly: true, setId: "templar", stats: { atk: 40, str: 5, dmg_holy: 15 } }, 
    ...
    "60697": { id: "60697", name: "Капюшон Тени", type: "head", icon: "🥋", rarity: "epic", lvl: 5, price: 800, allowedClasses: ["shadow", "ranger"], dropOnly: true, setId: "void", stats: { armor: 12, agi: 6, dodgeChance: 4, res_dark: 30 } },
    "47612": { id: "47612", name: "Пылающий Молот", type: "weapon1", icon: "🔨", rarity: "legendary", lvl: 6, price: 1100, allowedClasses: ["knight", "berserk"], dropOnly: true, setId: "bloodied", stats: { atk: 45, str: 8, armorPen: 15, dmg_fire: 25 } },
    ...
    "shields_v2_37": { id: "shields_v2_37", name: "Щит Инквизиции", type: "weapon2", icon: "🛡️", rarity: "epic", lvl: 6, price: 1400, allowedClasses: ["knight"], dropOnly: true, stats: { armor: 45, mst: 5, blockChance: 14, res_dark: 40 } },
    ...
    "shields_v2_38": { id: "shields_v2_38", name: "Эгида Доблести", type: "weapon2", icon: "🛡️", rarity: "legendary", lvl: 7, price: 2100, allowedClasses: ["knight"], dropOnly: true, setId: "templar", stats: { armor: 60, blockChance: 18, end: 6, res_dark: 50, dmg_holy: 10 } },
    "33616": { id: "33616", name: "Меч Инквизитора", type: "two_handed", icon: "🗡️", rarity: "legendary", premium: true, lvl: 8, price: 2500, allowedClasses: ["knight", "berserk"], dropOnly: true, setId: "bloodied", stats: { atk: 90, str: 15, critDmg: 25, dmg_holy: 30 } },
    ...
    "50560": { id: "50560", name: "Багровая Роба", type: "chest", icon: "🥋", rarity: "epic", lvl: 8, price: 2600, dropOnly: true, stats: { armor: 55, mst: 15, luk: 5, res_fire: 40 } },
    "17271": { id: "17271", name: "Рунный Пояс", type: "belt", icon: "➰", rarity: "legendary", lvl: 9, price: 3500, dropOnly: true, setId: "storm", stats: { armor: 15, end: 20, mst: 10, res_ice: 30 } },
    "18824": { id: "18824", name: "Сапфировое Кольцо", type: "ring", icon: "💍", rarity: "legendary", lvl: 9, price: 3000, dropOnly: true, setId: "storm", stats: { mst: 18, critChance: 10, dodgeChance: 5, dmg_ice: 25 } },
    "shields_v2_39": { id: "shields_v2_39", name: "Темный Бастион", type: "weapon2", icon: "🛡️", rarity: "legendary", premium: true, lvl: 9, price: 4000, allowedClasses: ["knight"], dropOnly: true, setId: "templar", stats: { armor: 90, atk: 25, blockChance: 20, end: -5, res_holy: -20, res_dark: 60 } },
    "50113": { id: "50113", name: "Одеяние Культиста", type: "chest", icon: "🥋", rarity: "legendary", lvl: 9, price: 3800, allowedClasses: ["shadow"], dropOnly: true, setId: "void", stats: { armor: 65, mst: 25, critChance: 8, res_holy: -20, res_dark: 50 } },
    ...
    "shields_v2_40": { id: "shields_v2_40", name: "Щит Дракона", type: "weapon2", icon: "🛡️", rarity: "legendary", lvl: 10, price: 6000, allowedClasses: ["knight"], dropOnly: true, stats: { armor: 120, blockChance: 25, end: 15, res_fire: 80, res_ice: -30 } }
};
...
function applyLoadedSave(savedHero, savedItems) {
    ...
    if(savedHero) { 
        ...
        // Безопасное восстановление инвентаря
...
function generateBossDrop(floor, specificId = null) {
    ...
    let pool = SHOP_ASSORTMENT.filter(id => ITEMS_DB[id] && ITEMS_DB[id].lvl <= maxLvl); 
    if(pool.length === 0) pool = SHOP_ASSORTMENT.filter(id => ITEMS_DB[id] && ITEMS_DB[id].lvl === 1); 
    baseId = pool[Math.floor(Math.random() * pool.length)]; 
...
function startPvP() {
    ...
    enemy = {
        ...
        nextAtkZone: ["head", "chest", "legs"][Math.floor(Math.random()*3)], turnCounter: 0,
...
function planEnemyTurn() {
    if(!enemy) return;
    enemy.turnCounter++; let delay = hasTalent('r2c') ? 3 : 0;
    if (enemy.isRaid) { let turnsLeft = (15 + delay) - enemy.turnCounter; if (turnsLeft <= 0) enemy.nextAtkZone = 'ENRAGE'; else if (enemy.turnCounter % 4 === 0) enemy.nextAtkZone = 'ULTIMATUM'; else enemy.nextAtkZone = ["head", "chest", "legs"][Math.floor(Math.random()*3)]; } 
    else { let ultMod = (enemy.turnCounter - delay) % 4; if (enemy.isBoss && ultMod === 0 && enemy.turnCounter > delay) enemy.nextAtkZone = 'ULTIMATUM'; else enemy.nextAtkZone = ["head", "chest", "legs"][Math.floor(Math.random()*3)]; }
    updateIntentDisplay();
}
...
function useClassSkill() {
    ...
    if (hero.baseClass === 'knight') { 
        ...
        let healPct = hasTalent('k3c') ? 0.50 : 0.25; let heal = Math.floor(hero.combatStats.hp * healPct); hero.hp = Math.min(hero.combatStats.hp, hero.hp + heal); 
        showDmgPopup("entity-hero-box", `+${heal} HP`, "log-sys"); logCombat(`<span class="log-skill">Вы применили СКИЛЛ! +${heal} HP.</span>`); 
    } 
    else if (hero.baseClass === 'berserk') { 
        triggerSkillVFX("entity-enemy-box", "vfx-berserk"); playLottieEffect("entity-enemy-box", VFX_DB.berserk_skill, "scale-huge"); shakeScreen(); 
        let dmg = Math.floor(hero.combatStats.damage * 2.5); enemy.hp -= dmg; if (enemy.isRaid) addQuestProgress('boss_dmg', dmg);
        showDmgPopup("entity-enemy-box", `-${dmg}`, "log-crit"); logCombat(`<span class="log-skill">Вы применили СКИЛЛ! -${dmg} HP.</span>`); playSFX('crit');
    } 
    else if (hero.baseClass === 'shadow') { 
        triggerSkillVFX("entity-hero-box", "vfx-shadow"); playLottieEffect("entity-enemy-box", VFX_DB.shadow_skill, "scale-up");
        let dmg = Math.floor(hero.combatStats.damage * 1.8); enemy.hp -= dmg; if (enemy.isRaid) addQuestProgress('boss_dmg', dmg);
        showDmgPopup("entity-enemy-box", `-${dmg}`, "log-crit"); logCombat(`<span class="log-skill">Вы применили СКИЛЛ! Уворот активен.</span>`); playSFX('hit');
    } 
    else if (hero.baseClass === 'ranger') { 
        triggerSkillVFX("entity-hero-box", "vfx-ranger"); playLottieEffect("entity-enemy-box", VFX_DB.ranger_skill, "scale-up");
        let dmg = Math.floor(hero.combatStats.damage * 1.5); enemy.hp -= dmg; if (enemy.isRaid) addQuestProgress('boss_dmg', dmg);
        combatState.enemyStunned = true; triggerHitAnim("entity-enemy-box"); showDmgPopup("entity-enemy-box", `ОГЛУШЕНИЕ!`, "log-block"); 
        if (hasTalent('r5a') && enemy.isBoss) { enemy.stats.armor = Math.floor(enemy.stats.armor * 0.9); logCombat(`<span class="log-skill">СКИЛЛ! Броня босса снижена на 10%.</span>`); } else { logCombat(`<span class="log-skill">Вы применили СКИЛЛ! Враг оглушен.</span>`); }
    }
...
function calcDmg(attacker, defender, zAtk, zDef, isHeroAtk) {
    ...
    if (bIsBerserk) {
        let missingHpPct = (hero.combatStats.hp - hero.hp) / hero.combatStats.hp; let stacks = Math.floor(missingHpPct * 10);
        if (hasTalent('b3b')) stacks *= 2; if (hero.flags.bloodied) stacks *= 2; 
        baseAtk = Math.floor(baseAtk * (1 + stacks * 0.05)); cChance += stacks * 2;
        if (hasTalent('b5b')) cDmg += Math.min(200, stacks * 20); 
        if (hasTalent('b3c')) armPen += Math.floor(defender.armor * 0.3); if (hasTalent('b5c')) armPen += defender.armor; 
        if (hasTalent('b2c') && combatState.zoneHealth[zAtk] === 0) baseAtk = Math.floor(baseAtk * 1.40); if (hasTalent('b1c')) combatState.zoneHealth[zAtk] = 0; 
    }
    ...
    ['fire', 'ice', 'dark', 'holy'].forEach(el => {
        ...
        if (rawElemDmg > 0) { let res = defender[`res_${el}`] || 0; let actualElemDmg = Math.floor(rawElemDmg * (1 - res/100)); if (actualElemDmg > 0) { elemDmgTotal += actualElemDmg; let icon = el==='fire'?'🔥':el==='ice'?'❄️':el==='dark'?'☠️':'☀️'; elemLog.push(`+${actualElemDmg}${icon}`); } }
    });
...
function handleCombatWin() {
    enemy.hp = 0; let lukDropMod = CLASSES[hero.baseClass].statWeights.luk_drop || 0.1; if(hero.flags.storm) lukDropMod *= 2;
    playSFX('win');
    ...
    if (combatMode === 'pvp') {
    ...
    else if (enemy.isRaid) {
        let gems = enemy.raidData.gemReward; if (hasTalent('r3b')) gems = Math.floor(gems * 1.5); hero.gems += gems; 
        let goldGained = hero.level * 50 + 100; hero.gold += goldGained; let droppedItem = null; let logMsg = "";
        if (enemy.raidData.dropRelic) {
            let pool = SHOP_ASSORTMENT.filter(id => ITEMS_DB[id] && ITEMS_DB[id].lvl <= hero.level); if(pool.length === 0) pool = SHOP_ASSORTMENT.filter(id => ITEMS_DB[id] && ITEMS_DB[id].lvl === 1);
            let baseId = pool[Math.floor(Math.random() * pool.length)]; let baseItem = ITEMS_DB[baseId]; let newItem = JSON.parse(JSON.stringify(baseItem));
            newItem.id = baseId + "_" + Date.now(); newItem.name = `✨ Древний ${baseItem.name}`; newItem.rarity = "relic"; newItem.dropOnly = true;
            for (let s in newItem.stats) newItem.stats[s] = Math.max(1, Math.ceil(newItem.stats[s] * 2));
            ITEMS_DB[newItem.id] = newItem; droppedItem = newItem;
            if (hero.inventory.length < 15) hero.inventory.push(droppedItem.id);
            else { hero.gold += 500; logMsg = `<br>Сумка полна! Реликвия продана.`; droppedItem = null; }
        }
        document.getElementById("vic-title-text").innerText = "РЕЙД ЗАВЕРШЕН!"; document.getElementById("vic-rewards-text").innerHTML = `Заработано: +${goldGained}💰 | +${gems}💎 ${logMsg}`;
        let lootBox = document.getElementById("vic-loot-container");
        if (lootBox) { if (droppedItem) { lootBox.style.display = "flex"; document.getElementById("vic-loot-box").className = `vic-loot-box rarity-${droppedItem.rarity}`; document.getElementById("vic-loot-box").innerHTML = renderItemIcon(droppedItem); document.getElementById("vic-loot-name").innerText = droppedItem.name; } else { lootBox.style.display = "none"; } }
    } else {
        ...
        if (hasTalent('r1b')) goldGained *= 2; hero.gold += goldGained; hero.exp += expGained;
        
        let dropChance = (enemy.isBoss ? 100 : (enemy.isMiniBoss ? 35 : 1)) + (hero.combatStats.luk * lukDropMod); 
        let logMsg = ""; let droppedItem = null;
        if (Math.random() * 100 <= dropChance) {
            let pool = SHOP_ASSORTMENT.filter(id => ITEMS_DB[id] && !ITEMS_DB[id].dropOnly); let dItem = ITEMS_DB[pool[Math.floor(Math.random() * pool.length)]];
            if (hero.inventory.length < 15) { hero.inventory.push(dItem.id); droppedItem = dItem; }
            else { hero.gold += Math.floor(dItem.price * 0.5); logMsg += `<br>Вещь продана за ${Math.floor(dItem.price * 0.5)}💰.`; }
        }
        if (hero.exp >= hero.expNext) { hero.exp -= hero.expNext; hero.level++; hero.unspentPoints += 3; hero.expNext = getExpReq(hero.level); calculateStats(); logMsg += `<br><span style="color:#34d399;">УРОВЕНЬ ПОВЫШЕН! +3 очка характеристик.</span>`; }
        
        document.getElementById("vic-title-text").innerText = "ЭТАЖ ПРОЙДЕН!"; document.getElementById("vic-rewards-text").innerHTML = `Заработано: +${goldGained}💰 | +${expGained} EXP ${logMsg}`;
        let lootBox = document.getElementById("vic-loot-container");
        if (lootBox) { if (droppedItem) { lootBox.style.display = "flex"; document.getElementById("vic-loot-box").className = `vic-loot-box rarity-${droppedItem.rarity}`; document.getElementById("vic-loot-box").innerHTML = renderItemIcon(droppedItem); document.getElementById("vic-loot-name").innerText = droppedItem.name; } else { lootBox.style.display = "none"; } }
...
function applyTurnEndEffects() {
    if (hasTalent('s1c') && hero.baseClass === 'shadow') {
        ...
        let dmgPerStack = Math.floor(enemy.maxHp * 0.05); if (hasTalent('s3c')) dmgPerStack = Math.floor(dmgPerStack * 1.5);
        let poisonDmg = dmgPerStack * combatState.poisonStacks; enemy.hp -= poisonDmg; if (enemy.isRaid) addQuestProgress('boss_dmg', poisonDmg);
...
function executeTurn() {
    ...
    if (hRes.type === "dodge") { playSFX('dodge'); } else if (hRes.type === "crit") { playSFX('crit'); shakeScreen(); } else if (hRes.type === "block" || hRes.type === "perfect_block") { playSFX('block'); } else { playSFX('hit'); }
    ...
    setTimeout(() => {
        ...
        if (enemy.hp <= 0) { 
            ...
            if (isEnemyStunned) { 
                ...
                combatState.enemyStunned = false; saveGame(); updateUI(); 
                isTurnExecuting = false; 
            } 
            else {
                setTimeout(() => {
                    ...
                    if (eAtkZone === 'ENRAGE') { eRes = { dmg: 99999, rawDmg: 99999, elemLog: '', type: "crit" }; shakeScreen(); } 
                        ...
                        else { let mitigation = Math.floor(hero.combatStats.armor * 0.2); let finalDmg = Math.max(Math.floor(baseAtk * 0.2), baseAtk - mitigation); eRes = { dmg: finalDmg, rawDmg: baseAtk, elemLog: '', type: "crit" }; } 
                    } else { 
                    ...
                    if (eRes.type === "dodge") { playSFX('dodge'); } else if (eRes.type === "crit" || eAtkZone === 'ENRAGE' || eAtkZone === 'ULTIMATUM') { playSFX('crit'); } else if (eRes.type === "block" || eRes.type === "perfect_block") { playSFX('block'); } else { playSFX('hit'); }
                    ...
                    setTimeout(() => {
                        ...
                        if (hero.hp <= 0 && !GOD_MODE) {
                            if (hero.flags.bloodied && !combatState.bloodiedUndying) { 
                                ...
                                logCombat(`<span class="log-sys">КРОВАВЫЙ ОСКАЛ! Вы выжили. След. удар лечит.</span>`); showDmgPopup("entity-hero-box", "ЖАЖДА!", "log-sys"); 
                                planEnemyTurn(); saveGame(); updateUI(); 
                                isTurnExecuting = false;
                            } 
                            else if (hasTalent('b5a') && hero.baseClass === 'berserk' && !combatState.undyingUsed) { 
                                hero.hp = 1; combatState.undyingUsed = true; 
                                logCombat(`<span class="log-sys">БЕССМЕРТИЕ! Вы выжили с 1 HP.</span>`); showDmgPopup("entity-hero-box", "СПАСЕН!", "log-sys"); 
                                planEnemyTurn(); saveGame(); updateUI(); 
                                isTurnExecuting = false;
                            } 
                            else { 
                                ...
                                if(combatMode === 'pvp') {
                                    ...
                                    logCombat(`<span class="log-dmg">Вы проиграли на Арене. Рейтинг -${ratingLost} 🏆</span>`);
...
function openInspectModal(invIndex) {
    ...
    if(item.type === 'consumable') { eqBtn.innerText = "ИСПОЛЬЗОВАТЬ В БОЮ"; eqBtn.style.background = "#52525b"; eqBtn.style.boxShadow = "none"; eqBtn.onclick = null; } 
    else { eqBtn.innerText = "НАДЕТЬ"; eqBtn.style.background = "#fbbf24"; eqBtn.style.boxShadow = "0 4px 10px rgba(251,191,36,0.3)"; eqBtn.onclick = function() { equipItem(inspectInvIndex); closeInspectModal(); }; }
...
function openScreen(screenName) {
    if (isTurnExecuting) return; // ЗАЩИТА: Блокируем смену вкладок во время анимации удара!

    if (!['hero', 'shop', 'classes', 'PVE', 'blacksmith', 'boss', 'talents', 'quests', 'arena', 'rating'].includes(screenName)) return alert("В разработке!");
    if ((combatMode === 'pvp' || combatMode === 'raid') && screenName !== 'PVE') { return alert("Сначала завершите текущий специальный бой!"); }
...
function equipItem(invIndex) {
    ...
    if ((targetSlot === 'weapon1' || targetSlot === 'weapon2') && hero.equipment.weapon1 && hero.equipment.weapon1.type === 'two_handed') {
        ...
        hero.inventory.splice(invIndex, 1); hero.inventory.push(twoHandedItem.id); hero.equipment[targetSlot] = item; calculateStats(); saveGame(); updateUI(); return;
    }
...
function healHero() {
    ...
    if (hero.gold < cost) { if (hero.gold > 0) { let affordableHeal = hero.gold * 2; hero.hp += affordableHeal; hero.gold = 0; alert(`Золота хватило лишь на частичное лечение (+${affordableHeal} HP).`); playSFX('coins'); saveGame(); updateUI(); } else { alert(`У вас нет золота! Продайте трофеи в Лавке или рискните спуститься на 1 этаж.`); } return; }
...
function upgradeItem() {
    if (forgeSelectedIndex === null) return; let itemId = hero.inventory[forgeSelectedIndex]; let item = ITEMS_DB[itemId]; let cost = item.lvl * item.price * 2; 
    if (hero.gold < cost) return alert("Не хватает золота!"); if (item.lvl >= 20) return alert("Максимальный уровень предмета достигнут!");
    hero.gold -= cost; let newItem = JSON.parse(JSON.stringify(item)); if (!newItem.id.includes("_upg_")) { newItem.id = newItem.id + "_upg_" + Date.now(); } else { newItem.id = newItem.id.split("_upg_")[0] + "_upg_" + Date.now(); }
    newItem.lvl += 1; newItem.price = Math.floor(newItem.price * 1.5); for (let s in newItem.stats) newItem.stats[s] = Math.max(1, Math.ceil(newItem.stats[s] * 1.15));
    ITEMS_DB[newItem.id] = newItem; hero.inventory[forgeSelectedIndex] = newItem.id; addQuestProgress('forge_upg', 1); if (window.tg && tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('success');
    let anvil = document.getElementById("forge-anvil"); if (anvil) { anvil.classList.remove("hammer-hit"); void anvil.offsetWidth; anvil.classList.add("hammer-hit"); } playSFX('forge'); saveGame(); updateUI();
}
function pickTalent(tierIndex, talentId) {
    let tData = TALENTS_DATA[hero.baseClass][tierIndex]; if (hero.level < tData.lvl) return alert(`Требуется ${tData.lvl} уровень!`);
    let tierTalentIds = tData.opts.map(o => o.id); if (hero.talents.some(t => tierTalentIds.includes(t))) return alert("Талант в этом тире уже выбран!");
    if(confirm("Вы уверены? Этот выбор навсегда определит стиль игры.")) { hero.talents.push(talentId); if (window.tg && tg.HapticFeedback) tg.HapticFeedback.notificationOccurred('success'); playSFX('skill'); calculateStats(); saveGame(); updateUI(); }
}
...
function updateUI() {
    ...
    if (currentScreen === 'arena') {
        ...
        if(elBoard) {
            let html = `
                <div class="pvp-player-card top-1"><div class="pvp-rank">1</div><img src="${STATIC_URL}berserk.png" class="pvp-avatar"><div class="pvp-info"><div class="pvp-name">Nagibator99</div><div class="pvp-stats">Берсерк • Ур. 84</div></div><div class="pvp-rating">🏆 4520</div></div>
                <div class="pvp-player-card top-2"><div class="pvp-rank">2</div><img src="${STATIC_URL}shadow.png" class="pvp-avatar"><div class="pvp-info"><div class="pvp-name">JohnWick</div><div class="pvp-stats">Тень • Ур. 79</div></div><div class="pvp-rating">🏆 3810</div></div>
                <div class="pvp-player-card top-3"><div class="pvp-rank">3</div><img src="${STATIC_URL}knight.png" class="pvp-avatar"><div class="pvp-info"><div class="pvp-name">Paladinus</div><div class="pvp-stats">Рыцарь • Ур. 75</div></div><div class="pvp-rating">🏆 3100</div></div>
    ...
    if (currentScreen === 'quests') {
        checkDailyQuests(); let html = '';
        for (let qId in DAILY_QUESTS) {
            let def = DAILY_QUESTS[qId]; let q = hero.quests[qId] || { progress: 0, claimed: false }; let pct = Math.min(100, (q.progress / def.target) * 100); let btnHtml = '';
            if (q.claimed) { btnHtml = `<div class="quest-btn claimed">ВЫПОЛНЕНО</div>`; } else if (q.progress >= def.target) { btnHtml = `<div class="quest-btn ready" onclick="claimQuest('${qId}')">ЗАБРАТЬ НАГРАДУ</div>`; } else { btnHtml = `<div class="quest-btn">${Math.floor(q.progress)} / ${def.target}</div>`; }
            html += `<div class="quest-card"><div class="quest-header"><span>${def.name}</span><span class="quest-reward">+${def.rewardGems} 💎</span></div><div class="quest-desc">${def.desc}</div><div class="quest-progress-wrap"><div class="quest-progress-fill" style="width: ${pct}%"></div></div>${btnHtml}</div>`;
        }
        let qList = document.getElementById("ui-quests-list"); if(qList) qList.innerHTML = html;
    }

    if (currentScreen === 'PVE' && enemy) {
        let titleEl = document.getElementById("combat-floor-title"); 
        if(titleEl) {
            if (combatMode === 'pvp' || combatMode === 'raid') { titleEl.style.display = 'none'; } 
            else { titleEl.style.display = 'flex'; titleEl.innerHTML = floorNavHtml; }
        }

        let stageNameEl = document.getElementById("combat-stage-name");
        if(stageNameEl) {
            if (combatMode === 'pvp') { stageNameEl.innerText = "PVP АРЕНА"; stageNameEl.className = "combat-header boss"; } 
            else if (combatMode === 'raid') { stageNameEl.innerText = "МИРОВОЙ БОСС"; stageNameEl.className = "combat-header boss"; } 
            else { stageNameEl.innerText = enemy.isBoss ? "МЕГА-БОСС" : (enemy.isMiniBoss ? "ЭЛИТНЫЙ ВРАГ" : "ОБЫЧНЫЙ ВРАГ"); stageNameEl.className = enemy.isBoss || enemy.isMiniBoss ? "combat-header boss" : "combat-header"; }
        }
        ...
        if (enemy.isBoss || enemy.isRaid) { 
            ...
            if(rPct >= 100) document.getElementById("enemy-rage-bar").classList.add("full"); else document.getElementById("enemy-rage-bar").classList.remove("full"); 
        }
        ...
        if (isTurnExecuting || combatState.skillCooldown > 0) { btnSkill.innerHTML = `<span style="font-size:7px; color:#d8b4fe">Скилл</span>КД (${combatState.skillCooldown})`; btnSkill.disabled = true; btnSkill.style.filter = "grayscale(100%) opacity(0.5)"; } 
        ...
        if (btnExe) { if(hero.hp <= 0 && !GOD_MODE) { btnExe.innerText = "ГЕРОЙ МЕРТВ"; btnExe.disabled = true; } else if (combatState.atkZone && combatState.defZone && !isTurnExecuting) { btnExe.innerText = "УДАРИТЬ ⚔️"; btnExe.disabled = false; } else { btnExe.innerText = "ВЫБЕРИТЕ ЗОНЫ"; btnExe.disabled = true; } }
    }
    ...
    if (currentScreen === 'hero') {
        ...
        for (let i = 0; i < 15; i++) { 
            if (i < hero.inventory.length) { 
                ...
                if (item) {
                    invHtml += `<div class="inv-item filled rarity-${item.rarity}" onclick="openInspectModal(${i})">${renderItemIcon(item)}</div>`; 
                } else {
                    invHtml += `<div class="inv-item empty"></div>`;
                }
            } else { 
                invHtml += `<div class="inv-item empty"></div>`; 
            } 
        ...
        let elemTable = `
            <div class="stat-resist-grid">
                <div style="font-size:10px;"><span class="elem-fire">🔥 Огонь:</span> <b>${hero.finalStats.dmg_fire}</b> | Рез: <b>${hero.finalStats.res_fire}%</b></div>
                <div style="font-size:10px;"><span class="elem-ice">❄️ Лед:</span> <b>${hero.finalStats.dmg_ice}</b> | Рез: <b>${hero.finalStats.res_ice}%</b></div>
                <div style="font-size:10px;"><span class="elem-dark">☠️ Тьма:</span> <b>${hero.finalStats.dmg_dark}</b> | Рез: <b>${hero.finalStats.res_dark}%</b></div>
                <div style="font-size:10px;"><span class="elem-holy">☀️ Свет:</span> <b>${hero.finalStats.dmg_holy}</b> | Рез: <b>${hero.finalStats.res_holy}%</b></div>
            </div>
        `;
    ...
    if (currentScreen === 'boss') {
        ...
        RAID_BOSSES.forEach(b => {
            ...
            let canAfford = hero.tickets > 0; let btnHtml = canAfford ? `<button class="raid-btn" onclick="startRaid('${b.id}')">В БОЙ (1 🎟️)</button>` : `<button class="raid-btn" disabled>НЕТ БИЛЕТОВ</button>`;
            ...
            let weakHtml = "";
            if (b.res_holy < 0) weakHtml = `<span class="elem-holy" style="font-size:9px;">Уязвим к Свету</span>`;
            if (b.res_ice < 0) weakHtml = `<span class="elem-ice" style="font-size:9px;">Уязвим ко Льду</span>`;
                ...
                <div class="raid-boss-card">
                    ...
                    <div class="raid-info">
                        <div>
                            ...
                            <div class="raid-desc">${b.desc} ${weakHtml}</div>
    ...
    if (currentScreen === 'blacksmith') {
        ...
        if (forgeSelectedIndex !== null && hero.inventory[forgeSelectedIndex]) {
            ...
            if (item) {
                ...
                if(btnUpg) { btnUpg.innerText = `КОВАТЬ (💰 ${cost})`; btnUpg.disabled = hero.gold < cost || item.lvl >= 20; if (item.lvl >= 20) btnUpg.innerText = "МАКСИМАЛЬНЫЙ УРОВЕНЬ"; }
    ...
    if (currentScreen === 'shop') {
        let shopHtml = '';
        if (shopMode === 'buy') {
            let filterEl = document.getElementById("filter-class");
            let fClass = filterEl ? filterEl.value : 'all';
            let sortedAssortment = [...SHOP_ASSORTMENT].filter(id => ITEMS_DB[id]).sort((a, b) => { 
                let itemA = ITEMS_DB[a], itemB = ITEMS_DB[b]; 
                if(itemA.lvl <= hero.level && itemB.lvl > hero.level) return -1; 
                if(itemA.lvl > hero.level && itemB.lvl <= hero.level) return 1; 
            ...
            sortedAssortment.forEach(itemId => {
                ...
                let price = getShopPrice(item.price); let canAfford = hero.gold >= price; let meetLvl = hero.level >= item.lvl; let classMatch = !item.allowedClasses || item.allowedClasses.includes(hero.baseClass);
